...
router.put('/draft', async (req, res) => {
  ...
  try {
    const sql = getSql();
    const [row] = await sql`
      INSERT INTO workflow_drafts (user_id, data, updated_at)
      VALUES (${userId}, ${jsonText(body.data)}::jsonb, now())
...
router.delete('/draft', async (req, res) => {
  ...
  try {
    const sql = getSql();
    await sql`DELETE FROM workflow_drafts WHERE user_id = ${userId}`;
    res.status(204).end();
  } catch (error) {
